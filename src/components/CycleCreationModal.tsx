...
import { getJsonContent, PlanData, updateTopicWeightAction } from '../app/actions';
...
const CycleCreationModal: React.FC<CycleCreationModalProps> = ({ isOpen, onClose, isEditing = false, initialData }) => {
  ...
  const [bancaTopicWeights, setBancaTopicWeights] = useState<PlanData['bancaTopicWeights'] | null>(null);
  ...
  useEffect(() => {
    async function loadSubjects() {
      if (selectedDataFile) {
        const data: PlanData = await getJsonContent(selectedDataFile);
        if (data) {
          const subjectsArray: Subject[] = data.subjects || [];
          const subjectsWithColors = subjectsArray.map((s: Subject) => ({
            ...s,
            color: s.color || '#94A3B8'
          }));
          setSubjects(subjectsWithColors);
          setBancaTopicWeights(data.bancaTopicWeights || null);
        }
      }
    }
    if (selectedDataFile) {
  ...
  const handleCalculateWeights = async () => {
    ...
    try {
      // 1. Crie uma lista plana de todos os tópicos com suas contagens das matérias selecionadas
      const topicsWithCounts = selectedSubjects.flatMap(subjectName => {
        const subject = subjects.find(s => s.subject === subjectName);
        if (!subject) return [];

        const topicList: { subjectName: string, topicText: string, count: number }[] = [];
        const traverse = (topics: Topic[]) => {
          topics.forEach(topic => {
            // Apenas considera tópicos que não são de agrupamento
            if (!topic.is_grouping_topic) {
              const count = bancaTopicWeights[subjectName]?.[topic.topic_text] ?? 0;
              topicList.push({ subjectName, topicText: topic.topic_text, count });
            }
            // Continua a recursão independentemente de ser agrupador ou não
            if (topic.sub_topics) {
      ...
      // 2. Separe os tópicos com 0 questões daqueles com > 0
      ...
      // 3. Ordene os tópicos por contagem de questões para preparar para o binning
      nonZeroCountTopics.sort((a, b) => a.count - b.count);

      // 4. Atribua pesos com base em quintis (5 caixas)
      const weightMap = new Map<string, number>();
      const total = nonZeroCountTopics.length;
      const binSize = Math.max(1, total / 5); // Garanta que o binSize seja pelo menos 1

      nonZeroCountTopics.forEach((topic, index) => {
        const weight = Math.min(5, Math.floor(index / binSize) + 1);
        const key = `${topic.subjectName}::${topic.topicText}`;
        weightMap.set(key, weight);
      });

      // Tópicos com 0 questões sempre recebem peso 1
      zeroCountTopics.forEach(topic => {
        const key = `${topic.subjectName}::${topic.topicText}`;
        weightMap.set(key, 1);
      });

      // 5. Atualize o estado dos subjects e prepare as promises para a atualização do backend
      const updatedSubjects = [...subjects];
      const updatePromises: Promise<any>[] = [];

      const updateTopicRecursively = (topics: Topic[], subjectName: string): Topic[] => {
        return topics.map(topic => {
          const key = `${subjectName}::${topic.topic_text}`;
          const newWeight = weightMap.get(key) ?? topic.userWeight ?? 1; // Fallback para o peso existente ou 1

          if (topic.userWeight !== newWeight) {
            if (selectedDataFile) {
              updatePromises.push(updateTopicWeightAction(selectedDataFile, subjectName, topic.topic_text, newWeight));
            }
          }

          const newSubTopics = topic.sub_topics ? updateTopicRecursively(topic.sub_topics, subjectName) : [];
      ...
      selectedSubjects.forEach(subjectName => {
        const subjectIndex = updatedSubjects.findIndex(s => s.subject === subjectName);
        if (subjectIndex !== -1) {
          updatedSubjects[subjectIndex].topics = updateTopicRecursively(updatedSubjects[subjectIndex].topics, subjectName);
        }
      });
