...
interface HierarchicalPerformanceNode {
    id: string;
    name: string;
    acertos: number;
    erros: number;
...
const calculateStats = async (
  ...
  const initializeTopicsRecursively = (topics: any[], parentTopicText: string = ''): EditalTopic[] => {
    return (topics || []).flatMap(topic => { // Use flatMap to potentially remove topics
      let cleanedTopicText = topic.topic_text;

      // Check if this topic's text is identical to its parent's text
      // This handles cases where a topic is just a repeat of its parent's name
      const isIdenticalToParent = (parentTopicText && cleanedTopicText === parentTopicText);

      // If parentTopicText exists and the current topic_text starts with it, remove the parent part
      ...
      // Remove leading " - " or similar separators more robustly
      ...
      const newTopic: EditalTopic = {
        ...topic,
        topic_text: cleanedTopicText, // Use the cleaned text
        completed: 0,
        reviewed: 0,
        total: 0,
        percentage: 0,
        last_study: '-',
        is_completed: false,
        sub_topics: topic.sub_topics ? initializeTopicsRecursively(topic.sub_topics, topic.topic_text) : [], // Pass original topic.topic_text as parent
      };
      return [newTopic]; // Return as an array for flatMap
    });
  };
  ...
  if (currentPlanData) {
    ...
    // Aplicar unicidade às matérias antes de processar
  ...
  const processTopicsRecursively = (topics: EditalTopic[], subjectName: string) => {
    topics.forEach(topic => {
      filteredStudyRecords.forEach(record => {
        if (record.subject === subjectName && record.topic === topic.topic_text) {
      ...
      if (topic.sub_topics && topic.sub_topics.length > 0) {
        processTopicsRecursively(topic.sub_topics, subjectName);
      }
    });
  };

  editalData.forEach(subject => {
    processTopicsRecursively(subject.topics, subject.subject);
  });

  // Função para agregar estatísticas dos filhos para os pais
  const aggregateStatsRecursively = (topic: EditalTopic): { completed: number, total: number, last_study: string, is_completed: boolean } => {
    if (!topic.sub_topics || topic.sub_topics.length === 0) {
      return {
        ...
        total: topic.total,
        last_study: topic.last_study,
        is_completed: topic.is_completed
      };
    }

    let aggregatedCompleted = 0;
    ...
    topic.sub_topics.forEach(subTopic => {
      ...
      if (childStats.last_study !== '-') {
        if (mostRecentStudy === '-' || new Date(childStats.last_study) > new Date(mostRecentStudy)) {
          mostRecentStudy = childStats.last_study;
        }
      }
      if (!childStats.is_completed) {
  ...
  editalData.forEach(subject => {
    subject.topics.forEach(topic => aggregateStatsRecursively(topic));
  });
  ...
  const processHierarchicalPerformance = (
      ...
      return topics.map(topic => {
          ...
          // Recursively process sub_topics first to get their aggregated data
          const childrenPerformanceNodes = topic.sub_topics
              ? processHierarchicalPerformance(topic.sub_topics, subjectName, studyRecords)
              : [];

          let acertos = 0;
          let erros = 0;
          let naoFeitas = 0; // Placeholder for now

          if (isGroupingTopic) {
              // Grouping topics ONLY aggregate data from children
              ...
              // Non-grouping topics calculate their own performance from records
              studyRecords.filter(record =>
                  record.subject === subjectName && record.topic === topic.topic_text
              ).forEach(record => {
                  if (record.questions) {
          ...
          return {
              ...
              children: childrenPerformanceNodes.length > 0 ? childrenPerformanceNodes : undefined,
          };
      });
  };

  const hierarchicalTopicPerformance: HierarchicalPerformanceNode[] = editalData.map(subject => {
      const children = processHierarchicalPerformance(subject.topics, subject.subject, filteredStudyRecords);
      ...
      const countLeaves = (nodes: HierarchicalPerformanceNode[]): { acertos: number, erros: number, naoFeitas: number } => {
        ...
        nodes.forEach(node => {
          if (!node.children || node.children.length === 0) {
            acertos += node.acertos;
            erros += node.erros;
            naoFeitas += node.naoFeitas;
          } else {
            const childSums = countLeaves(node.children);
            acertos += childSums.acertos;
            erros += childSums.erros;
            naoFeitas += childSums.naoFeitas;
          }
        });
      ...
      return {
          id: subject.subject, // Using subject name as id
...
const calculateTopicScores = (
  ...
  const getAllTopicsRecursively = (subjects: EditalSubject[]): { subject: string, topic: string, userWeight: number }[] => {
    const topicList: { subject: string, topic: string, userWeight: number }[] = [];
    const dive = (subjectName: string, topics: EditalTopic[]) => {
      for (const topic of topics) {
        topicList.push({
          subject: subjectName,
        ...
        if (topic.sub_topics && topic.sub_topics.length > 0) {
          dive(subjectName, topic.sub_topics);
        }
      }
    };
    for (const subject of subjects) {
      dive(subject.subject, subject.topics);
    }
    return topicList;
  };
